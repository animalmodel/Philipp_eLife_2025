...
%
% PURPOSE: 
%   Generates Figure 9: Synergy Cross-Correlation Analysis.
%   Layout: 2 Rows x 2 Columns per Monkey (Total 8 Panels).
%
% VISUAL STYLE:
%   - Top Row (Blue): Post-surgery vs Pre-surgery Synergy B (Extensor).
%   - Bottom Row (Red): Post-surgery vs Pre-surgery Synergy A (Flexor).
...
% Output Directory
outFigDir = fullfile(baseDir, 'outputFigures_Fig9');
...
% Colors
colorTop = [0.2 0.2 0.8]; % Blue (vs Syn B)
colorBot = [0.8 0.2 0.2]; % Red  (vs Syn A)
...
figPos = [100, 100, scrsz(3)*0.5, scrsz(4)*0.8];
fig = figure('Name', 'Figure 9: Synergy Correlation', 'Position', figPos, 'Color', 'w');
...
for mIndx = 1:2
    ...
    % --- TOP ROW (BLUE) ---
    % Col 1: Synergy A vs Pre-B (Idx 2)
    ax1 = nexttile(1 + rowOffset);
    plotOverlay(ax1, data_All, data_Mean, 2, dataBeh, colorTop, [-1 1], currLMD, currXTicks, xLimVal);
    if mIndx == 1, title(ax1, 'Synergy A', 'FontSize', 14, 'FontWeight', 'bold'); end
    ...
    % Col 2: Synergy B vs Pre-B (Idx 6)
    ax2 = nexttile(2 + rowOffset);
    plotOverlay(ax2, data_All, data_Mean, 6, dataBeh, colorTop, [-1 1], currLMD, currXTicks, xLimVal);
    if mIndx == 1, title(ax2, 'Synergy B', 'FontSize', 14, 'FontWeight', 'bold'); end
    text(ax2, 0.5, 0.2, 'vs Syn B (pre)', 'Units', 'normalized', 'Color', colorTop, 'FontSize', 9);
    
    % --- BOTTOM ROW (RED) ---
    % Col 1: Synergy A vs Pre-A (Idx 1)
    ax3 = nexttile(3 + rowOffset);
    plotOverlay(ax3, data_All, data_Mean, 1, dataBeh, colorBot, [-1 1], currLMD, currXTicks, xLimVal);
    text(ax3, 0.5, 0.2, 'vs Syn A (pre)', 'Units', 'normalized', 'Color', colorBot, 'FontSize', 9);
    
    % Col 2: Synergy B vs Pre-A (Idx 5)
    ax4 = nexttile(4 + rowOffset);
    plotOverlay(ax4, data_All, data_Mean, 5, dataBeh, colorBot, [-1 1], currLMD, currXTicks, xLimVal);
...
% Save
try
    print(fig, fullfile(outFigDir, 'Figure9_SynergyCorr.svg'), '-dsvg', '-painters');
    fprintf('Figure 9 saved to: %s\n', outFigDir);
catch
    exportgraphics(fig, fullfile(outFigDir, 'Figure9_SynergyCorr.png'), 'Resolution', 300);
    fprintf('SVG save failed. Figure available in PNG at: %s\n', outFigDir);
end
disp('Figure 9 generated successfully.');
